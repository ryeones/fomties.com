...
export default {
  ...
  components: {
    ...
    contentMeta: { readingTime: ({ minutes, words }) => `đọc ${minutes} phút (${words} từ)` },
  },
  pages: {
    rss: { recentNotes: 'Những bài gần đây', lastFewNotes: ({ count }) => `${count} Bài gần đây` },
    error: { title: 'Không Tìm Thấy', notFound: 'Trang này được bảo mật hoặc không tồn tại.' },
    folderContent: {
      folder: 'Thư Mục',
