...
export interface Translation {
  ...
  components: {
    ...
    graph: { title: string }
    recentNotes: { title: string; seeRemainingMore: (variables: { remaining: number }) => string }
    transcludes: {
    ...
    contentMeta: { readingTime: (variables: { minutes: number; words: number }) => string }
  }
  pages: {
    rss: { recentNotes: string; lastFewNotes: (variables: { count: number }) => string }
    error: { title: string; notFound: string }
    folderContent: { folder: string; itemsUnderFolder: (variables: { count: number }) => string }
    tagContent: {
      tag: string
      tagIndex: string
